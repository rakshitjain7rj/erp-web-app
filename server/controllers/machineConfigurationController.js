  ...
  try {
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      if (saveHistory === true) {
        ...
        // Check if the new config is significantly different from the active one
        // to avoid trivial updates (e.g., formatting differences)
        ...
        if (activeConfig) {
          ...
          if (significantChange) {
            ...
            console.log('Skipping configuration update - no significant changes detected');
          }
        }
        
        // Only create a new configuration if there's a significant change or no active config
        if (!activeConfig || significantChange) {
          ...
          // Return the existing active config if no significant changes
          return activeConfig;
        }
      }
