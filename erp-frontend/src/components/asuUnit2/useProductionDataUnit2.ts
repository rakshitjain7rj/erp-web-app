...
import { asuUnit2Api, ASUProductionEntryUnit2 } from '../../api/asuUnit2Api';
import { ASUMachine, ProductionStats } from '../../api/asuUnit1Api';
...
export interface UseProductionDataReturn {
    machines: ASUMachine[];
    productionEntries: ASUProductionEntryUnit2[];
...
export const useProductionDataUnit2 = (): UseProductionDataReturn => {
    const [machines, setMachines] = useState<ASUMachine[]>([]);
    const [productionEntries, setProductionEntries] = useState<ASUProductionEntryUnit2[]>([]);
    ...
    const loadData = useCallback(async (machineId?: number, dateFilter?: DateFilter) => {
        ...
        try {
            ...
            const [fetchedMachines, fetchedEntries, fetchedStats] = await Promise.all([
                ...
                asuUnit2Api.getProductionEntries(filters),
                asuUnit2Api.getProductionStats()
            ]);
    ...
    const addEntry = useCallback(async (data: any, machineId?: number) => {
        ...
        const optimisticEntry: ASUProductionEntryUnit2 = {
            id: optimisticId,
            ...
            dayMainsReading: Number(data.dayMainsReading) || 0,
            nightMainsReading: Number(data.nightMainsReading) || 0
        };
        ...
        try {
            const createdEntry = await asuUnit2Api.createProductionEntry(data);
            
            // Replace optimistic entry with real one (no reload needed!)
            setProductionEntries(prev => prev.map(e => 
                e.id === optimisticId ? createdEntry : e
            ));
    ...
    const updateEntry = useCallback(async (id: number, data: any, machineId?: number) => {
        ...
        const optimisticEntry: ASUProductionEntryUnit2 = {
            ...
            updatedAt: new Date().toISOString(),
            // Update new fields
            dayShiftWorker: data.dayShiftWorker !== undefined ? data.dayShiftWorker : originalEntry.dayShiftWorker,
            nightShiftWorker: data.nightShiftWorker !== undefined ? data.nightShiftWorker : originalEntry.nightShiftWorker,
            dayMainsReading: data.dayMainsReading !== undefined ? Number(data.dayMainsReading) : originalEntry.dayMainsReading,
            nightMainsReading: data.nightMainsReading !== undefined ? Number(data.nightMainsReading) : originalEntry.nightMainsReading
        };
        ...
        try {
            ...
            await asuUnit2Api.updateProductionEntry(id, data, {
    ...
    const deleteEntry = useCallback(async (id: number, machineId?: number) => {
        ...
        try {
            await asuUnit2Api.deleteProductionEntry(id);
            return true;
        } catch (err: any) {
