...
const ASUUnit: React.FC<ASUUnitProps> = ({ unit }) => {
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      try {
        ...
        if (dailyRes.success) setDailyMachineData(dailyRes.data);
        if (productionRes.success) setProductionEfficiency(productionRes.data);
        if (mainsRes.success) setMainsReadings(mainsRes.data);
  ...
  const tabs = [
    { label: 'Daily Machine Data', value: 'daily', icon: <Activity className="w-4 h-4" /> },
    { label: 'Production Efficiency', value: 'production', icon: <TrendingUp className="w-4 h-4" /> },
    { label: 'Mains Readings', value: 'mains', icon: <Zap className="w-4 h-4" /> },
    { label: 'Weekly Data', value: 'weekly', icon: <Calendar className="w-4 h-4" /> },
  ] as const;
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">ASU Unit {unit} Tracking</h1>
