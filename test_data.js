const { ASUProductionEntry, ASUMachine } = require('./server/models/ASUModels');
  ...
  try {
    ...
    const entries = await ASUProductionEntry.findAll({
      ...
      include: [{
        model: ASUMachine,
        as: 'machine',
        attributes: ['id', 'machineNo', 'productionAt100', 'count', 'yarnType']
      }]
    });
    ...
    machines.forEach(machine => {
      console.log(`Machine ${machine.id}:`, {
        ...
        productionAt100: machine.productionAt100,
        count: machine.count,
